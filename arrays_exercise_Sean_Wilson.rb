...
if categories.include? 'Animal' && 'Shopping'
  categories << "Family"
end
...
first_category = categories.sort.shift
categories.delete('Animals')
...
last_category = categories.delete 'Family'
last_category = categories.sort.pop
...
if categories.length > 5 #get it's length
  categories.delete_if { categories.length >= 0 }  # empty the array
  categories << "Misc"
end
